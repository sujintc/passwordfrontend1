...
function Home() {
  ...
  const handleRegBtn = async () => {
    ...
    https://passwordbackend1.onrender.com


    try {


      const regResponce = await fetch('https://passwordbackend1.onrender.com/api/user/register', {
  ...
  const handleLoginBtn = async () => {
    ...
    const loginRes = await fetch("https://passwordbackend1.onrender.com/api/user/login", {
  ...
  const handleForget = async () => {
    ...
    try {
      ...
      const forgetFetch = await fetch('https://passwordbackend1.onrender.com/api/resetpassword', {
  ...
  const fetchdata = async () => {
    ...
    try {
      ...
      const responce = await fetch("https://passwordbackend1.onrender.com/api/getuser", {
      ...
      <div className="main container-fluid text-center py-3">
        ...
        <div className="register container-fluid d-flex flex-column align-items-center justify-content-center ">
          ...
          <div className="form-group mt-3 mx-5 ">
            <input className="form-control " type="text" value={RegUserName} onChange={handleRegUserInp} placeholder='Enter usename' />

            <input className="form-control mt-3" type="text" value={RegEmailId} onChange={handleRegEmailInp} placeholder='Enter EmailId' />

            <input className="form-control mt-3" type="text" value={RegPassword} onChange={handleRegPasswordInp} placeholder='Enter Password' />

            <button className='btn btn-primary mt-3' onClick={handleRegBtn}>register</button>
          </div>
